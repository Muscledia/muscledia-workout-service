...
public class WorkoutAnalyticsController {
    ...
    public Flux<WorkoutAnalyticsResponse> getHistoricalAnalytics(
        ...
        return authenticationService.getCurrentUserId()
                .flatMapMany(userId -> analyticsService.getHistoricalAnalytics(userId, period.toUpperCase(), periods))
                .doOnComplete(() -> log.info("Retrieved {} periods of {} historical analytics", periods, period));
    }
    ...
    public Mono<ResponseEntity<ProgressTrackingResponse>> getExerciseProgress(
        ...
        return authenticationService.getCurrentUserId()
                .flatMap(userId -> progressTrackingService.getExerciseProgress(userId, exerciseId, days))
                .map(ResponseEntity::ok)
                .switchIfEmpty(Mono.just(ResponseEntity.notFound().build()))
                .doOnSuccess(response -> log.info("Retrieved progress for exercise {} over {} days", exerciseId, days));
    }
    ...
    public Mono<ResponseEntity<Boolean>> checkPotentialPR(
        ...
        return authenticationService.getCurrentUserId()
                .flatMap(userId -> personalRecordService.wouldBePR(userId, exerciseId, recordType.toUpperCase(), value))
                .map(ResponseEntity::ok)
                .doOnSuccess(response -> log.info("Checked potential PR for exercise {} type {} value {}",
